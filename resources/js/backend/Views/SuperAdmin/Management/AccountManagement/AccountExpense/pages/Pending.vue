          ...
          <div class="card-body">
            <div class="table-responsive table_responsive card_body_fixed_height">
              <table class="table table-hover text-center table-bordered">
                ...
                <tbody>
                  <tr v-for="item in all?.data" :key="item.id" :class="`table_rows table_row_${item.id}`">
                    <td>
                    ...
                    <td>{{ item.id }}</td>
                    <td>{{ item.account_category?.title.substr(50) }}</td>
                    <td>{{ item.title.substr(50) }}</td>
                    <td>{{ item.amount }}</td>
                    <td>
...
export default {
  ...
  methods: {
    ...
    updateStatus: async function (event, item) {
      let conf = await window.s_confirm("Are you sure you want to update the status?");
      if (!conf) {
